...
my %net_form=(
 data=>[
  ...
  {ftype=>1, len=>42, maxlen=>39, tag=>'range_start', name=>'Range start', type=>'ip',
   empty=>1, iff=>['subnet','t']},
  {ftype=>1, len=>42, maxlen=>39, tag=>'range_end', name=>'Range end', type=>'ip',
   empty=>1, iff=>['subnet','t']},
...
sub show_net_record($$)
{
  ...
  print hidden('net_id',$id),end_form,' ';
...
#	   "dummy FROM nets " .
  ...
  if (@q < 1) {
    print '<div class="s-empty-state">',
          '<p class="s-empty-state__title">No networks found</p>',
          '<p class="s-empty-state__hint">No networks are defined for this server.</p>',
          '</div>';
    return;
  }
  if (check_perms('level',$main::ALEVEL_VLANS,1)) {
...
#			    ($novlans ? '' : $vlan), $q[$i][8], $pc], 1) . "\n";
  ...
  for $i (0..$#q) {
    ...
    if ($q[$i][0] == -1) {
	# Synthetic "free hole" row generated by the net browser.
	print '<tr class="s-list__row" data-net-state="free">';
	$dhcp = $type = '&nbsp;';
    } elsif ($q[$i][3] =~ /(1|t)/) {
	if ($q[$i][9] =~ /(1|t)/) {
	    print '<tr class="s-list__row" data-net-state="virtual">';
	    $type = 'Virtual';
	} else {
	    print '<tr class="s-list__row" data-net-state="',
	          ($dhcp eq 'Yes' ? 'dhcp' : 'static'), '">';
	    $type = 'Subnet';
	}
    } else {
	print '<tr class="s-list__row" data-net-state="net">';
	$type = 'Net';
    }
...
sub menu_handler {
  ...
  if ($sub eq 'vmps') {
    ...
    print '<div class="s-net-info">';
    print '<div class="s-net-info__form">';
    ...
    print '</div><div class="s-net-info__blocks">';
...
# Free block, if any, after last used address.
	...
	print '<table class="s-list">',
	      '<tr class="s-list__head"><th colspan="3">Net usage map</th></tr>',
	      '<tr class="s-list__head">', td("Size"), td("Start"), td("End"), '</tr>';
	my $log10 = log(10);
	for $i (0..$#blocks) {
	    print '<tr class="s-list__row" data-block-state="',
	          ($blocks[$i][3] == 1 ? 'used' : 'free'), '">';
	    if ($blocks[$i][0] < 1e6) {
		print td("$blocks[$i][0] &nbsp;");
	    } else {
	...
	print "</table>";
	print p, '<table class="s-list">',
	      '<tr class="s-list__head"><th colspan="3">Legend</th></tr>',
	      '<tr class="s-list__row" data-block-state="used"><td>In use</td></tr>',
	      '<tr class="s-list__row" data-block-state="free"><td>Unused</td></tr>',
	      "</table>";
    }
	...
	$hid=$netmap{$ip}->[0];
	if ($nmaphash{$ip} =~ /^Up/) {
	  $status='<span class="s-badge--success">UP</span>';
	} else {
	  $status='<span class="s-badge--danger">DOWN ' . encode_entities($nmaphash{$ip}) . '</span>';
	}
	$domain=$netmap{$ip}->[1];
	$domain="UNKNOWN" unless ($domain);
	$domain="<a href=\"$selfurl?menu=hosts&h_id=$hid\">".$domain."</a>"
	  if ($hid > 0);
	$info = join_strings(', ',(@{$netmap{$ip}})[6,3,4,5]);
